...
def logo():
    os.system("clear")
    print(f"""{white}[{green}>>{white}] Author  : Muhammad Nazri XD{white}
{white}[{green}>>{white}] Contact : +6281221523195{white}  """)
    print(50*'-')
...
def file_crack():
    ...
    print(f"[{green}+{white}] example : sayang - kontol - memekk")
    print(50*'-')
    ...
    with ThreadPool(max_workers=30) as BilalHaiderID:
        ...
        logo()
        print(f"[{green}*{white}] Total ID Yang Di Crack : {total}")
        print(f"[{green}*{white}] Total Password Crack : {str(len(pwx))}")
        print(f"[{green}*{white}] File : {fileX}")
        print(50*'-')
